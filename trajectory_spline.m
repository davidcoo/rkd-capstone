...
for way = 1:num_joints
    interpolated_times = linspace(times(1), times(end), sum(num_points_per_segment));
    trajectory(way,:) = spline(times, [0 waypoints(way,:) 0], interpolated_times);%zeros(num_joints, sum(num_points_per_segment)); % Replace this with actual value of trajectory!
end
% --------------- END STUDENT SECTION ------------------------------------
